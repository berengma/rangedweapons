...
minetest.register_node("rangedweapons:antigun_block", {
		...
		"Prevents people from using guns, in 10 node radius to each side"..
		" from this block"),
...
make_sparks = function(pos)
	...
	for i=1,9 do
		minetest.add_particle({
			pos = pos, 
			velocity = {x=math.random(-6.0,6.0),  y=math.random(-10.0,15.0),  
				z=math.random(-6.0,6.0)},
        	acceleration = {x=math.random(-9.0,9.0), y=math.random(-15.0,-3.0),
...
rangedweapons_yeet = function(itemstack, player)
	...
	local throw_cooldown = ThrowCaps.throw_cooldown or 0
	local skill_value = 1
	local throw_damage = ThrowCaps.throw_damage or {fleshy=1}
	local throw_sound = ThrowCaps.throw_sound or "rangedweapons_glock"
	local throw_velocity = ThrowCaps.throw_velocity or 20
	local throw_accuracy = ThrowCaps.throw_accuracy or 100
	local throw_cooling = ThrowCaps.throw_cooling or itemstack:get_name()
	local throw_crit = ThrowCaps.throw_crit or 0
	local throw_critEffc = ThrowCaps.throw_critEffc or 1
	local throw_projectiles = ThrowCaps.throw_projectiles or 1
	local throw_mobPen = ThrowCaps.throw_mob_penetration or 0
	local throw_nodePen = ThrowCaps.throw_node_penetration or 0
	local throw_dps = ThrowCaps.throw_dps or 0
	local throw_gravity = ThrowCaps.throw_gravity or 0
	local throw_door_breaking = ThrowCaps.throw_door_breaking or 0
	local throw_ent = ThrowCaps.throw_entity or "rangedweapons:shot_bullet"
	local throw_visual = ThrowCaps.throw_visual or "wielditem"
	local throw_texture = ThrowCaps.throw_texture or "rangedweapons:shot_bullet_visual"
	local throw_glass_breaking = ThrowCaps.throw_glass_breaking or 0
	local throw_particles = ThrowCaps.throw_particles or nil
	local throw_sparks = ThrowCaps.throw_sparks or 0
	local throw_bomb_ignite = ThrowCaps.ignites_explosives or 0
	local throw_size = ThrowCaps.throw_projectile_size or 0
	local throw_glow = ThrowCaps.throw_projectile_glow or 0
	local OnCollision = ThrowCaps.OnCollision or function()end
	local throw_skill = ThrowCaps.throw_skill[1] or ""
	local throw_skillChance = ThrowCaps.throw_skill[2] or 0

	if minetest.find_node_near(player:getpos(), 10,"rangedweapons:"..
	"antigun_block") then
		minetest.chat_send_player(player:get_player_name(), "" ..
			core.colorize("#ff0000","throwable weapons are prohibited"..
				" in this area!"))
		return
	end
	if playerMeta:get_float("rw_cooldown") <= 0 then
		...
		rangedweapons_launch_projectile(player,throw_projectiles,
			throw_damage,throw_ent,throw_visual,throw_texture,
			throw_sound,throw_velocity,throw_accuracy,skill_value,
			OnCollision,throw_crit,throw_critEffc,throw_mobPen,
			throw_nodePen,0,"","","",throw_dps,throw_gravity,
			throw_door_breaking, throw_glass_breaking,throw_particles,
			throw_sparks,throw_bomb_ignite, throw_size,0,
			itemstack:get_wear(),throw_glow)
		itemstack:take_item()
	end
end
...
rangedweapons_shoot_gun = function(itemstack, player)
	...
	if gunMeta:get_int("RW_bullets") > 0 and 
		...
		rangedweapons_launch_projectile(player,combined_projNum,combined_dmg,
			bullet_ent,bullet_visual,bullet_texture,gun_sound,
			combined_velocity,gun_accuracy,skill_value,OnCollision,
			combined_crit,combined_critEffc,combined_mobPen,combined_nodePen,
			gun_shell,bullet_shell_ent,bullet_shell_texture,
			bullet_shell_visual,combined_dps,combined_gravity,
			gun_door_breaking,bullet_glass_breaking,bullet_particles,
			bullet_sparks,bullet_bomb_ignite,bullet_size,gun_smokeSize,0,
			bullet_glow)
...
rangedweapons_shoot_powergun = function(itemstack, player)
	...
	local power_cooldown = PowerCaps.power_cooldown or 0
	local power_consumption = PowerCaps.power_consumption or 0
	local inv = player:get_inventory()
	local playerMeta = player:get_meta()
	local OnCollision = function() end
	...
	playerMeta:get_float("rw_cooldown") <= 0 then
		...
		power_texture = PowerCaps.power_texture or "rangedweapons:"..
			"shot_bullet_visual"
	...
	rangedweapons_launch_projectile(player,power_projectiles,power_damage,
		power_ent,power_visual,power_texture,power_sound,power_velocity,
		power_accuracy,skill_value,OnCollision,power_crit,power_critEffc,
		power_mobPen,power_nodePen,0,"","","",power_dps,power_gravity,
		power_door_breaking,power_glass_breaking,power_particles,power_sparks,
		power_bomb_ignite,power_size,0,0,power_glow)
	if rweapons_gun_durability == "true" then
	...
	inv:remove_item("main", "rangedweapons:power_particle "..
		PowerCaps.power_consumption)
end

rangedweapons_launch_projectile = function(player,projNum,projDmg,projEnt,
	visualType,texture,shoot_sound,combined_velocity,accuracy,skill_value,
	ColResult,projCrit,projCritEffc,mobPen,nodePen,has_shell,shellEnt,
	shellTexture,shellVisual,dps,gravity,door_break,glass_break,
	...
	if not pos or not dir or not yaw then
		return
	end
	...
	for i = 1, projectiles do
		...
		if obj then
			...
			obj:set_acceleration({x=math.random(-acc,acc), y=math.random(-acc, 0)-gravity, z=math.random(-acc,acc)})
...
minetest.register_globalstep(function(dtime, player)
	rangedweapons.timer = rangedweapons.timer + dtime;
	if rangedweapons.timer >= 1.0 then
